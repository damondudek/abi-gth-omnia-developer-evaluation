...
public class CreateUserRequestValidator : AbstractValidator<CreateUserRequest>
{
    ...
    public CreateUserRequestValidator()
    {
        ...
        RuleFor(user => user.Name).NotNull().ChildRules(children =>
        {
            children.RuleFor(name => name.FirstName).NotEmpty().Length(3, 50);
            children.RuleFor(name => name.LastName).NotEmpty().Length(3, 50);
        });
